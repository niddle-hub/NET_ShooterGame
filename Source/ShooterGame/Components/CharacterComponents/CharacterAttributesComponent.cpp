...
#include "ShooterGame/Subsystems/KillfeedSubsystem.h"
...
void UCharacterAttributesComponent::OnTakeAnyDamage(AActor* DamagedActor, float Damage, const UDamageType* DamageType, AController* InstigatedBy, AActor* DamageCauser)
{
	...
	{
		const auto Killfeed = UGameplayStatics::GetGameInstance(this)->GetSubsystem<UKillfeedSubsystem>();
		Killfeed->RegisterKillEvent(CachedBaseCharacterOwner->GetController(), InstigatedBy, DamageCauser);
